...
class gui_tkinter_widgets():
    ...
    def __init__(self, master, tipo_widget_param = None, entorno_donde_se_llama_la_clase = None, **kwargs_config_parametros):
        ...
        --> entorno_donde_se_llama_la_clase\n
        Es la clase padre donde se usa en el proyecto de GUI donde se incorpora este modulo (el self)
        o el módulo Python si la llamada se hace fuera de una clase propia.
        ...
        self.entorno_donde_se_llama_la_clase = entorno_donde_se_llama_la_clase


        #se inicializa el atributo propio self.modulos_python_importados_entorno_llamada_clase_madre
        #que almacena los modulos python importados en el modulo donde se realiza la llamada a la presente clase
        #es necesario para poder usar rutinas / funciones pasadas como string en los kwargs
        #cuando dichas rutinas / funciones estan en modulos python importados en el modulo de la GUI
        inspect_currentframe = inspect.currentframe()
        llamada_inspect_currentframe = inspect_currentframe.f_back
        globals_llamada_inspect_currentframe = llamada_inspect_currentframe.f_globals

        self.modulos_python_importados_entorno_llamada_clase_madre = {nombre_objeto: objeto for nombre_objeto, objeto in globals_llamada_inspect_currentframe.items()
                                                                        if isinstance(objeto, types.ModuleType)
                                                                        }
        del inspect_currentframe
        del llamada_inspect_currentframe
        del globals_llamada_inspect_currentframe
        ...
        self.nombre_kwargs_combobox_opciones_editables = "combobox_opciones_editables"

        self.nombre_kwargs_dicc_rutina = "dicc_rutina"
        self.nombre_kwargs_dicc_rutina_modulo_python_alias = "modulo_python_alias"
        ...
        self.nombre_kwargs_lista_dicc_rutina_aplicar_eventos_widget_modulo_python_alias = "modulo_python_alias"
        ...
        self.nombre_kwargs_dicc_imagen_tupla_pixeles_imagen= "tupla_pixeles_imagen"
        ...
        self.nombre_kwargs_lista_dicc_rutina_trace_variable_enlace_modulo_python_alias = "modulo_python_alias"
        ...
        self.tupla_pixeles_imagen_por_defecto = (20, 20)
    ...
    def config_atributos(self, **kwargs_config_parametros):
        ...
        if len(self.kwargs_config_parametros_lower_no_blank) != 0:
                ...
                try:
                    ...
                    if isinstance(dicc_config_root, dict):
                        ...
                        if self.tipo_widget_lower_no_blank == "toplevel":
                            ...
                            #ya configurado el toplevel se muestra en pantalla 
                            #(esto se combina con self.widget_objeto.withdraw() cuando se crea el objeto)
                            self.widget_objeto.update_idletasks()
                            self.widget_objeto.deiconify()
                except:
                    pass
            ...
            # combobox_opciones_editables
            if self.nombre_kwargs_combobox_opciones_editables in lista_config_atributos_propios:

                try:
                    opcion_editar = self.kwargs_config_parametros_lower_no_blank.get(self.nombre_kwargs_combobox_opciones_editables, False)

                    if opcion_editar:
                        self.widget_objeto["state"] = "normal"
            ...
            if self.nombre_kwargs_dicc_rutina in lista_config_atributos_propios:
                ...
                try:
                    ...
                    if isinstance(dicc_rutina, dict):
                        ...
                        modulo_python_alias = dicc_rutina.get(self.nombre_kwargs_dicc_rutina_modulo_python_alias, None)
                        nombre_rutina = dicc_rutina.get(self.nombre_kwargs_dicc_rutina_nombre_rutina, None)
                        parametros_args = dicc_rutina.get(self.nombre_kwargs_dicc_rutina_parametros_args, ())
                        parametros_kwargs = dicc_rutina.get(self.nombre_kwargs_dicc_rutina_parametros_kwargs, {})


                        if isinstance(nombre_rutina, str) and self.entorno_donde_se_llama_la_clase is not None:

                            modulo_python = self.modulos_python_importados_entorno_llamada_clase_madre.get(modulo_python_alias) if isinstance(modulo_python_alias, str) else None

                            if modulo_python:
                                rutina_objeto = getattr(modulo_python, nombre_rutina, None)
                            else:
                                rutina_objeto = getattr(self.entorno_donde_se_llama_la_clase, nombre_rutina, None)
                        ...
                        if callable(rutina_objeto):
                            ...
                            self.widget_objeto.config(command = lambda: rutina_objeto(
                                                                                        #args dinamicos
                                                                                        *(arg(self.entorno_donde_se_llama_la_clase) if callable(arg) else arg for arg in parametros_args)

                                                                                        #kwargs dinamicos
                                                                                        , **{key: (valor(self.entorno_donde_se_llama_la_clase) if callable(valor) else valor)
                                                                                                for key, valor in parametros_kwargs.items()}
                                                                                    )
                                                    )
            ...
            # --> 'modulo_python_alias' (atributo self.nombre_kwargs_lista_dicc_rutina_aplicar_eventos_widget_modulo_python_alias)
                ...
                try:
                    ...
                    if (isinstance(lista_dicc_rutina_aplicar_eventos_widget, list)
                        ...
                        for dicc in lista_dicc_rutina_aplicar_eventos_widget:
                            ...
                            modulo_python_alias = dicc.get(self.nombre_kwargs_lista_dicc_rutina_aplicar_eventos_widget_modulo_python_alias, None)
                            nombre_rutina = dicc.get(self.nombre_kwargs_lista_dicc_rutina_aplicar_eventos_widget_rutina, None)
                            parametros_args = dicc.get(self.nombre_kwargs_lista_dicc_rutina_aplicar_eventos_widget_rutina_parametros_args, ())
                            parametros_kwargs = dicc.get(self.nombre_kwargs_lista_dicc_rutina_aplicar_eventos_widget_rutina_parametros_kwargs, {})
                            ...
                            if callable(rutina_objeto):
                                ...
                                # IMPORTANTE: poner la "captura_lambda" en "captura_lambda = rutina_objeto" y "parametros_kwargs = parametros_kwargs: captura_lambda("
                                ...
                                self.widget_objeto.bind(tipo_bind
                                                        ...
                                                        , parametros_kwargs = parametros_kwargs: captura_lambda(
                                                                                                                # args dinamicos
                                                                                                                *(arg(self.entorno_donde_se_llama_la_clase) if callable(arg) else arg
                                                                                                                for arg in parametros_args),

                                                                                                                # kwargs dinamicos
                                                                                                                **{key: (valor(self.entorno_donde_se_llama_la_clase)
            ...
            # --> 'modulo_python_alias' (atributo self.nombre_kwargs_lista_dicc_rutina_trace_variable_enlace_modulo_python_alias)
                ...
                try:
                    ...
                    if isinstance(lista_dicc_rutina_trace_variable_enlace, list) and sum(1 if isinstance(dicc, dict) else 0 for dicc in lista_dicc_rutina_trace_variable_enlace) == len(lista_dicc_rutina_trace_variable_enlace):
                        ...
                        for dicc in lista_dicc_rutina_trace_variable_enlace:
                            ...
                            modulo_python_alias = dicc.get(self.nombre_kwargs_lista_dicc_rutina_trace_variable_enlace_modulo_python_alias, None)
                            ...
                            parametros_args = dicc.get(self.nombre_kwargs_lista_dicc_rutina_trace_variable_enlace_parametros_args, ())
                            parametros_kwargs = dicc.get(self.nombre_kwargs_lista_dicc_rutina_trace_variable_enlace_parametros_kwargs, {})
            ...
            if self.nombre_kwargs_dicc_imagen in lista_config_atributos_propios:
                ...
                if isinstance(self.kwargs_config_parametros_lower_no_blank[self.nombre_kwargs_dicc_imagen], dict):
                    ...
                    tupla_pixeles_imagen = self.kwargs_config_parametros_lower_no_blank[self.nombre_kwargs_dicc_imagen].get(self.nombre_kwargs_dicc_imagen_tupla_pixeles_imagen, self.tupla_pixeles_imagen_por_defecto)

                    try:
                        #se redimensiona la imagen sino los widgets salen distorsionados en la gui
                        #se usa el kwargs tupla_pixeles_imagen
                        img = Image.open(png_imagen).resize(tupla_pixeles_imagen, Image.LANCZOS)
    ...
    def __varios_clase_madre(self, opcion_varios: Literal ["crear_widget_objeto", "nombre_modulo_python_py", "nombre_alias_libreria_python_import", "objeto_alias_libreria_python_import", 
        ...
        if opcion_varios == "crear_widget_objeto":
            ...
            if check_localiz_modulo_y_objeto == "ok":
                ...
                if self.tipo_widget_lower_no_blank == "toplevel":
                    #impide que se muestre aun el toplevel pq sino aparece brevemente al crearse
                    #sin configurar con el icono de pluma
                    self.widget_objeto.withdraw()
...
class frame_con_scrollbar(gui_tkinter_widgets):
    ...
    def __init__(self, master, **kwargs_config_widget):
        ...
        if self.kwargs_dicc_frame_scrollbar_reconstruido_usado_creacion is not None:
...
class entry_propio(gui_tkinter_widgets):
    ...
    def __init__(self, master, entorno_donde_se_llama_la_clase = None, **kwargs_config_widget):
        

        #se habilita la herencia atributos y metodos de la clase madre
        super().__init__(master, entorno_donde_se_llama_la_clase = entorno_donde_se_llama_la_clase, **kwargs_config_widget)
        ...
        self.formato_validacion_lower_no_blank = None
        ...
        self.funcion_validacion_personalizada = None
        self.modulo_python_alias = None
        self.resultado_funcion_validacion_personalizada_para_bloquear_exit = None
        self.validacion_personalizada_messagebox_warning = None

        self.integrar_boton_calendario = False



        #se inicializa el atributo propio self.modulos_python_importados_entorno_llamada_clase_entry_propio
        #que almacena los modulos python importados en el modulo donde se realiza la llamada a la presente clase
        #es necesario para poder usar rutinas / funciones pasadas como string en los kwargs
        #cuando dichas rutinas / funciones estan en modulos python importados en el modulo de la GUI
        inspect_currentframe = inspect.currentframe()
        ...
        self.modulos_python_importados_entorno_llamada_clase_entry_propio = {nombre_objeto: objeto for nombre_objeto, objeto in globals_llamada_inspect_currentframe.items()
                                                                            if isinstance(objeto, types.ModuleType)
                                                                            }
        del inspect_currentframe
        del llamada_inspect_currentframe
        ...
        self.nombre_kwargs_dicc_entry_modulo_python_alias = "modulo_python_alias"
    ...
    def __varios_clase_hija(self, opcion_varios: Literal["crear_widget_objeto"
        ...
        if opcion_varios == "crear_widget_objeto":
            ...
            try:
                self.dicc_entry = self.kwargs_config_widget.get(self.nombre_kwargs_dicc_entry, None)

                self.formato_validacion = self.dicc_entry.get(self.nombre_kwargs_dicc_entry_formato_validacion, None) if isinstance(self.dicc_entry, dict) else None
                self.formato_validacion_lower_no_blank = self.formato_validacion.lower().replace(" ", "") if self.formato_validacion is not None else None

                self.texto_longitud_maxima = self.dicc_entry.get(self.nombre_kwargs_dicc_entry_texto_longitud_maxima, None) if isinstance(self.dicc_entry, dict) else None
                self.titulo_messagebox_warning = self.dicc_entry.get(self.nombre_kwargs_dicc_entry_titulo_messagebox_warning, "") if isinstance(self.dicc_entry, dict) else ""

                self.calendario_tupla_coord_place_y_width = self.dicc_entry.get(self.nombre_kwargs_dicc_entry_calendario_tupla_coord_place_y_width, None) if isinstance(self.dicc_entry, dict) else None
                self.calendario_iconbitmap = self.dicc_entry.get(self.nombre_kwargs_dicc_entry_calendario_iconbitmap, None) if isinstance(self.dicc_entry, dict) else None


                self.funcion_validacion_personalizada = self.dicc_entry.get(self.nombre_kwargs_dicc_entry_funcion_validacion_personalizada, None) if isinstance(self.dicc_entry, dict) else None
                self.modulo_python_alias = self.dicc_entry.get(self.nombre_kwargs_dicc_entry_modulo_python_alias, None) if isinstance(self.dicc_entry, dict) else None
                
                self.resultado_funcion_validacion_personalizada_para_bloquear_exit = (self.dicc_entry.get(self.nombre_kwargs_dicc_entry_resultado_funcion_validacion_personalizada_para_bloquear_exit, None)
                                                                                if isinstance(self.dicc_entry, dict) else None)
                
                self.validacion_personalizada_messagebox_warning = self.dicc_entry.get(self.nombre_kwargs_dicc_entry_messagebox_warning_validacion_personalizada, "") if isinstance(self.dicc_entry, dict) else None



                #se localiza si el tipo de validacion es interno a la clase (mediante el atributo dicc_patrones_validacion) o es personalizada por el usuario
                #mediante una funcion que el usuario define en su proyecto
                # --> si se configuran tanto el caso de validacion interna a la clase como el caso de validacion personalizada por el usuario premia la personalizada
                # --> si ni la validacion interna ni la personalizada estan bien configuradas se crea un entry normal sin reglas de validacion
                tipo_validacion = ("definida_por_el_usuario"
                                    if isinstance(self.funcion_validacion_personalizada, str) 
                                        and self.resultado_funcion_validacion_personalizada_para_bloquear_exit is not None
                                        and self.validacion_personalizada_messagebox_warning is not None

                                    else "definida_en_la_clase"
                                    if isinstance(self.formato_validacion, str) 
                                        and self.formato_validacion.lower().replace(" ", "") in list(self.dicc_patrones_validacion.keys())
                                    else None
                                    )
                ...
                if tipo_validacion is not None:
                    self.widget_objeto.bind("<FocusOut>", lambda event: self.__exit_entry(tipo_validacion, event))
                ...
                self.integrar_boton_calendario = (True if isinstance(self.calendario_tupla_coord_place_y_width, tuple) 
                                                            ...
                                                            and len(self.calendario_tupla_coord_place_y_width) == 3
                                                            and "fecha" in self.formato_validacion.lower().replace(" ", "")
                                                    else False)
                
                if self.integrar_boton_calendario:
            ...
            date_pattern = "dd/mm/yyyy" if self.formato_validacion_lower_no_blank == "fecha_ddmmaaaa" else "yyyy/mm/dd"
    ...
    def __exit_entry(self, tipo_validacion: Literal["definida_en_la_clase", "definida_por_el_usuario"],  event = None):
        ...
        try:
            #se recupera el valor informado en el widget entry y no se ejcutan ni los CASOS 1 ni los casos 2 si el entry no esta informado
            ...
            #############################################################################
            # CASO 1 - validacion interna a la clase
            #############################################################################
            if tipo_validacion == "definida_en_la_clase":


                #se recuperan los datos del diccionario dicc_patrones_validacion
                validacion_re = self.dicc_patrones_validacion[self.formato_validacion_lower_no_blank]["validacion_re"]
                mensaje_warning = self.dicc_patrones_validacion[self.formato_validacion_lower_no_blank]["mensaje_warning"]
                ...
                if self.formato_validacion_lower_no_blank == "texto":
                    ...
                    if not re.fullmatch(validacion_re, valor_entry):
                        ...
                        if self.formato_validacion_lower_no_blank == "texto" and isinstance(self.texto_longitud_maxima, (int, float)) and self.texto_longitud_maxima > 0:
            ...
            #############################################################################
            # CASO 2 - validacion personalizada por el usuario
            #############################################################################
            elif tipo_validacion == "definida_por_el_usuario":
                
                if isinstance(self.funcion_validacion_personalizada, str) and self.entorno_donde_se_llama_la_clase is not None:

                    modulo_python = self.modulos_python_importados_entorno_llamada_clase_entry_propio.get(self.modulo_python_alias) if isinstance(self.modulo_python_alias, str) else None

                    if modulo_python:
                        rutina_objeto = getattr(modulo_python, self.funcion_validacion_personalizada, None)
                    else:
                        rutina_objeto = getattr(self.entorno_donde_se_llama_la_clase, self.funcion_validacion_personalizada, None)

                else:
                    rutina_objeto = self.funcion_validacion_personalizada


                if callable(rutina_objeto):
                    resultado_funcion_validacion_personalizada = rutina_objeto(valor_entry)

                    if resultado_funcion_validacion_personalizada == self.resultado_funcion_validacion_personalizada_para_bloquear_exit:
                        messagebox.showwarning(title = self.titulo_messagebox_warning, message = self.validacion_personalizada_messagebox_warning)
                        self.widget_objeto.focus_set()
...
class treeview_propio(gui_tkinter_widgets):
    ...
    def __init__(self, master, entorno_donde_se_llama_la_clase = None, **kwargs_config_widget):
        ...
        self.widget_objeto = None
        self.datos_items = None


        #se inicializa el atributo propio self.modulos_python_importados_entorno_llamada_clase_treeview_propio
        #que almacena los modulos python importados en el modulo donde se realiza la llamada a la presente clase
        #es necesario para poder usar rutinas / funciones pasadas como string en los kwargs
        #cuando dichas rutinas / funciones estan en modulos python importados en el modulo de la GUI
        inspect_currentframe = inspect.currentframe()
        ...
        self.modulos_python_importados_entorno_llamada_clase_treeview_propio = {nombre_objeto: objeto for nombre_objeto, objeto in globals_llamada_inspect_currentframe.items()
                                                                                if isinstance(objeto, types.ModuleType)
                                                                                }
        del inspect_currentframe
        del llamada_inspect_currentframe
        ...
        self.nombre_kwargs_dicc_treeview_rutina_click_item_modulo_python_alias = "modulo_python_alias"
    ...
    def acciones(self, opcion: Literal["actualizar_desde_df", "eliminar_contenido", "modificar_objeto", "seleccionar_item"], **kwargs):

        """
        Método que permite realizar acciones varias en un treeview.
        ...
        --> eliminar_contenido\n
        Permite eliminar el contendo y flags del treeview.
        ...
        --> seleccionar_item\n
        Permite seleccionar un item en el treeview (o varios si el treeview es de selección multiple).
        ...
        --> lista_item_seleccionado\n
        Se usa tan solo si se ha optado por el parámetro args 'seleccionar_item'. Es lista de listas donde cada sublista correponde a un item seleccionado.
        """

        #parametros kwargs
        df_datos = kwargs.get("df_datos", None)
        lista_item_seleccionado = kwargs.get("lista_item_seleccionado", None)
        ...
        if opcion == "actualizar_desde_df":
            ...
            if self.widget_objeto is not None:
                ...
                if isinstance(df_datos, pd.DataFrame):
                    ...
                    try:
                        lista_config_columnas_df = self.datos_items["lista_columnas_df"]
                        ...
                        #en datos_items (lista_columnas_df)
                        #se informa el atributo propio de la clase self.datos_items["dicc_registros_treeview_con_iid"]
                        self.datos_items["dicc_registros_treeview_con_iid"] = {}
                        if len(df_datos) != 0 and len(lista_columnas_df_datos_ok) != 0:
                            ...
                            self.datos_items["lista_tipo_dato_columna_df"] = lista_columnas_df_tipo_datos


                            #se rellena el treeview y se informa el el atributo propio self.datos_items["dicc_registros_treeview_con_iid"]
                            #se resetean los indices de df_datos para poder iterar sobre los mismos para rellenar el treeview
                            ...
                            for ind in df_datos.index:
                                str_iid = str(ind)
                                lista_item = [df_datos.iloc[ind, df_datos.columns.get_loc(col)] for col in lista_config_columnas_df]
                                tupla_item = tuple(lista_item)

                                self.widget_objeto.insert("", "end", iid = str_iid, values = tupla_item)
                                self.datos_items["dicc_registros_treeview_con_iid"].update({str_iid: lista_item})


                        elif len(df_datos) == 0:
                            #se vacia el treeview y se reinicia el atributo propio self.datos_items["dicc_registros_treeview_con_iid"]
                            self.datos_items["dicc_registros_treeview_con_iid"] = None
        ...
        elif opcion == "eliminar_contenido":
            ...
            dicc_treeview_rutina_click_item_usado_creacion_modulo_python_alias = dicc_treeview_rutina_click_item_usado_creacion.get(self.nombre_kwargs_dicc_treeview_rutina_click_item_modulo_python_alias, None)
            ...
            dicc_treeview_rutina_click_item_modificaciones_modulo_python_alias = dicc_treeview_rutina_click_item_modificaciones.get(self.nombre_kwargs_dicc_treeview_rutina_click_item_modulo_python_alias, None)
            ...
            check_si_volver_a_crear_treeview = (True if dicc_treeview_usado_creacion_seleccion_item != dicc_treeview_modificaciones_seleccion_item
                                                        ...
                                                        or dicc_treeview_rutina_click_item_usado_creacion_modulo_python_alias != dicc_treeview_rutina_click_item_modificaciones_modulo_python_alias
        ...
        elif opcion == "seleccionar_item":

            if isinstance(lista_item_seleccionado, list) and isinstance(self.datos_items["dicc_registros_treeview_con_iid"], dict):

                try:
                    lista_iid = [iid for sublista_1 in lista_item_seleccionado
                                for iid, sublista_2 in self.datos_items["dicc_registros_treeview_con_iid"].items()
                                if sublista_1 == sublista_2]

                    self.widget_objeto.selection_set(*lista_iid) #selection_set espera args de ahi el "*"
                    self.widget_objeto.focus(lista_iid[0])
    ...
    def __varios_clase_hija(self, opcion: Literal["crear_widget_objeto", "crear_widget_objeto_desde_modificaciones"], **kwargs_modificaciones):
        ...
        try:
            ...
            width_acum = 2#se pone 2 pq sino el borde del treeview no sale en la derecha del titulo de la ultima columna
            ...
            #se crea el atributo propio datos_items con los datos de configuracion del treeview
            ...
            # --> lista_datos_items_seleccionados          es la lista de los datos de los items seleccionados (es lista de lista)
            # --> dicc_registros_treeview_con_iid          es un diccionario donde cada key es el iid del registro del treeview y su valor una lista con el item correpondiente
            self.datos_items = {"lista_columnas_df": list(columnas_df)
                                ...
                                , "lista_datos_items_seleccionados": None          #se asigna cuando se realiza la accion de click sobre un item del treeview
                                , "dicc_registros_treeview_con_iid": None          #se asigna cuando se realiza la accion 'actualizar_desde_df' (metodo propio acciones)
                                }
            ...
            # --> 'modulo_python_alias' (atributo self.nombre_kwargs_dicc_treeview_rutina_click_item_modulo_python_alias)
            # --> 'rutina' (atributo self.nombre_kwargs_dicc_treeview_rutina_click_item_nombre_rutina)
            # --> 'parametros_args' (atributo self.nombre_kwargs_dicc_treeview_rutina_click_item_paprintrametros_args)
            ...
            try:
                if isinstance(dicc_rutina_click_item, dict):
                    ...
                    modulo_python_alias = dicc_rutina_click_item.get(self.nombre_kwargs_dicc_treeview_rutina_click_item_modulo_python_alias, None)
                    nombre_rutina = dicc_rutina_click_item.get(self.nombre_kwargs_dicc_treeview_rutina_click_item_nombre_rutina, None)
                    parametros_args = dicc_rutina_click_item.get(self.nombre_kwargs_dicc_treeview_rutina_click_item_parametros_args, ())
                    parametros_kwargs = dicc_rutina_click_item.get(self.nombre_kwargs_dicc_treeview_rutina_click_item_parametros_kwargs, {})

                    if isinstance(nombre_rutina, str) and self.entorno_donde_se_llama_la_clase is not None:

                        modulo_python = self.modulos_python_importados_entorno_llamada_clase_treeview_propio.get(modulo_python_alias) if isinstance(modulo_python_alias, str) else None
                ...
                self.kwargs_usados_creacion_antes_modificaciones[self.nombre_kwargs_dicc_treeview] = kwargs_usados.get(self.nombre_kwargs_dicc_treeview, None)
                self.kwargs_usados_creacion_antes_modificaciones[self.nombre_kwargs_dicc_treeview_rutina_click_item] = kwargs_usados.get(self.nombre_kwargs_dicc_treeview_rutina_click_item, None)
    ...
    def __click_on_item(self, event = None):
        ...
        self.datos_items["lista_datos_items_seleccionados"] = [list(self.widget_objeto.item(item_id, "values")) for item_id in item_selecc] if item_selecc else None
...
class scrolledtext_propio(gui_tkinter_widgets):
    ...
    def texto_informado(self, opcion: Literal["todo", "entre_lineas"], **kwargs):
        """Función que permite recuperar el texto informado en un scrolledtext.

        args
        --
        --> todo\n
            Recupera todo el texto informado en el scrolledtext.

        --> entre_lineas\n
            Recupera texto informado en el scrolledtext entre lineas que se indican en los kwargs.

        kwargs
        --
        Solo aplican para la opción 'entre_lineas'.\n

        --> linea_desde\n
        --> linea_hasta\n
        """

        linea_desde = kwargs.get("linea_desde", None)
        linea_hasta = kwargs.get("linea_hasta", None)

        return (self.widget_objeto.get("1.0", "end-1c")
                if opcion == "todo"
                else self.widget_objeto.get(f"{linea_desde}.0", f"{linea_hasta}.end")
                if opcion == "entre_lineas" 
                    and isinstance(linea_desde, int)
                    and isinstance(linea_hasta, int)
                    and linea_desde >= 0
                    and linea_hasta >= 0
                else None 
                )
...
class messagebox_propio:
    """Clase independiente (sin herencias) que permite generar messagebox como los nativos tkinter pero con el condicionante
    de que solo se puede generar uno a la vez (los nativos de tkinter si pulsas varias veces un botón que genera un messagebox
    salen en pantalla tantos messagebox como clicks realizados).
    """
    ...
    def __init__(self, master, **kwargs_config_widget):
        ...
        #se incializa el atributo que sirve para recuperar el valor del boton pulsado fuera de la clase
        self.valor_boton_pulsado = None


        #se asigna el nombre del diccionario kwargs donde recuperar los parametros de configuracion
        #(esto es por si se desea cambiar en el futuro el nombre el mismo sin tener que modificar el codigo de la clase)
        self.nombre_kwargs_messagebox_propio_tipo_messagebox = "tipo_messagebox"
        self.nombre_kwargs_messagebox_propio_iconbitmap = "iconbitmap"
        self.nombre_kwargs_messagebox_propio_titulo = "titulo"
        self.nombre_kwargs_messagebox_propio_mensaje = "mensaje"

        self.nombre_kwargs_messagebox_propio_dicc_imagen = "dicc_imagen"
        self.nombre_kwargs_messagebox_propio_dicc_imagen_png_imagen_messagebox = "png_imagen_messagebox"
        self.nombre_kwargs_messagebox_propio_dicc_imagen_tupla_pixeles_imagen = "tupla_pixeles_imagen"

        self.nombre_kwargs_messagebox_propio_font_messagebox = "font_messagebox"
        self.nombre_kwargs_messagebox_propio_bg_messagebox = "bg_messagebox"
        self.nombre_kwargs_messagebox_propio_fg_messagebox = "fg_messagebox"

        self.nombre_kwargs_messagebox_propio_font_boton = "font_boton"
        self.nombre_kwargs_messagebox_propio_bg_boton = "bg_boton"
        self.nombre_kwargs_messagebox_propio_fg_boton = "fg_boton"


        #se inicializan como atributos los kwargs de la clase ademas de otros
        self.tipo_messagebox = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_tipo_messagebox, "showinfo")
        self.iconbitmap = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_iconbitmap, None)
        self.titulo = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_titulo, "")
        self.mensaje = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_mensaje, "")

        self.dicc_imagen = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_dicc_imagen, {})
        self.png_imagen_messagebox = self.dicc_imagen.get(self.nombre_kwargs_messagebox_propio_dicc_imagen_png_imagen_messagebox, None)
        self.tupla_pixeles_imagen = self.dicc_imagen.get(self.nombre_kwargs_messagebox_propio_dicc_imagen_tupla_pixeles_imagen, (23, 23))

        self.font_messagebox = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_font_messagebox, None)
        self.bg_messagebox = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_bg_messagebox, None)
        self.fg_messagebox = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_fg_messagebox, None)

        self.font_boton = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_font_boton, None)
        self.bg_boton = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_bg_boton, None)
        self.fg_boton = kwargs_config_widget.get(self.nombre_kwargs_messagebox_propio_fg_boton, None)

        self.font_por_defecto = ("Arial", 9)



        
        #se incializa el diccionario dicc_config_messagebox con los pameretos para recrear los messagebox nativos de tkinter
        #cada key corresponde a un tipo de messagebox nativo de tkinter (se pueden agregar otros personalizados)
        #los valores de estas keys son diccionarios con las keys siguientes:
        # --> bitmap                es el string que permite recuperar la imagen a incrustar en el messagebox sin necesidad de disponer de un png
        # --> lista_botones         es lista de listas donde cada sublista contiene:
        #                                                                         --> string para el literal del boton
        #                                                                         --> valor que devuelve el boton al pulsarlo
        # --> marcar_foco_boton     es el boton en el cual se desea marcar el foco (el string ha de estar incluido en la lista de la key lista_botones)
        self.dicc_config_messagebox = {"showinfo":
                                                    {"bitmap": "info"
                                                    , "lista_botones": [["Aceptar", True]]
                                                    , "marcar_foco_boton": "Aceptar"
                                                    }
                                        ...
                                        , "askokcancel":
                                                    {"bitmap": "question"
                                                    , "lista_botones": [["Aceptar", True], ["Cancelar", False]]
                                                    , "marcar_foco_boton": "Aceptar"
                                                    }

                                        , "askokcancel_warning":#se adapta el modulo UTILS solo en este caso para el uso especifico del app
                                                    {"bitmap": "warning"
                                                    , "lista_botones": [["Aceptar", True], ["Cancelar", False]]
                                                    , "marcar_foco_boton": "Aceptar"
                                                    }

                                        , "askokcancel_con_opciones_colocacion_pantallazos":#se adapta el modulo UTILS solo en este caso para el uso especifico del app
                                                    {"bitmap": "warning"
                                                    , "lista_botones": [["COORD\n(sup-izq)", True], ["COORD\n(hibrido)", False], ["Cancelar", None]]
                                                    , "marcar_foco_boton": "ok (sin)"
                                                    }

                                        , "askokcancel_con_opciones_guardado_config_id_pptx":#se adapta el modulo UTILS solo en este caso para el uso especifico del app
                                                    {"bitmap": "warning"
                                                    , "lista_botones": [["Guardar\n(sin)", True], ["Guardar\n(con)", False], ["Cancelar", None]]
                                                    , "marcar_foco_boton": "ok (sin)"
                                                    }

                                        , "askokcancel_con_o_sin_muestra_screenshots":#se adapta el modulo UTILS solo en este caso para el uso especifico del app
                                                    {"bitmap": "warning"
                                                    , "lista_botones": [["Activar", True], ["Desactivar", False], ["Cancelar", None]]
                                                    , "marcar_foco_boton": "ok (con)"
                                                    }
                                        ...
                                        , "askretrycancel":
                                                    {"bitmap": "question"
                                                    , "lista_botones": [["Reintentar", True], ["Cancelar", False]]
                                                    , "marcar_foco_boton": "Reintentar"
                                                    }

                                        , "askyesno":
                                                    {"bitmap": "question"
                                                    , "lista_botones": [["Si", True], ["No", False]]
                                                    , "marcar_foco_boton": "Si"
                                                    }

                                        , "askyesnocancel":
                                                    {"bitmap": "question"
                                                    , "lista_botones": [["Si", True], ["No", False], ["Cancelar", None]]
                                                    , "marcar_foco_boton": "Si"
                                                    }
                                        }
        ...
        self.__varios_clase("crear_messagebox", tipo_messagebox = self.tipo_messagebox)



    def __varios_clase(self, opcion: Literal["objeto_alias_libreria_python_import", "crear_messagebox", "contenido", "botones", "cerrar", "centrar"], **kwargs):
        ...
        --> cerrar_messagebox
        \t\tCierra el messagebox y asocia un valor al boton pulsado

        kwargs
        --
        --> libreria_python (solo para opcion = 'objeto_alias_libreria_python_import')\n
        --> tipo_messagebox (solo para opcion = 'crear_messagebox')\n
        --> boton_valor (solo para opciones = 'crear_messagebox' y 'cerrar_messagebox')
        """

        resultado_funcion = None

        libreria_python = kwargs.get("libreria_python", None)
        tipo_messagebox = kwargs.get("tipo_messagebox", None)
        boton_valor = kwargs.get("boton_valor", None)
        kwargs_master = kwargs.get("kwargs_master", None)
        color_check = kwargs.get("color_check", None)
        ...
        if opcion == "objeto_alias_libreria_python_import":
        ...
        elif opcion == "crear_messagebox":
            #crea el messagebox

            #se recupera el diccionario de self.dicc_config_messagebox asociado al kwargs tipo_messagebox
            #para poder recuperar el bitmap (para asociar una imagen al tipo de messagebox) y la lista de botones a crear (con su valor)
            dicc_tipo_messagebox = self.dicc_config_messagebox.get(tipo_messagebox, {})
            bitmap = dicc_tipo_messagebox.get("bitmap", None)
            lista_botones = dicc_tipo_messagebox.get("lista_botones", None)
            marcar_foco_boton = dicc_tipo_messagebox.get("marcar_foco_boton", None)


            #se ajustan los kwargs de configuracion (bg, fg y font)
            #se localizan si estan correctamente configurados (en caso de no serlo se les atribuye el valor estandar)
            #es necesario pq se han de configurar en el momento de la creacion de los widgets que componen el messagebox
            #(en casxo contrario si se configuran colores distintos al estandar el topllevel sale de color estandar y el resto no)
            bg_messagebox_ajust = self.bg_messagebox if self.__varios_clase("check_si_color_correcto", kwargs_master = self.master, color_check = self.bg_messagebox) else "#FFFFFF"
            fg_messagebox_ajust = self.fg_messagebox if self.__varios_clase("check_si_color_correcto", kwargs_master = self.master, color_check = self.fg_messagebox) else "black"

            bg_boton_ajust = self.bg_boton if self.__varios_clase("check_si_color_correcto", kwargs_master = self.master, color_check = self.bg_boton) else "#FFFFFF"
            fg_boton_ajust = self.fg_boton if self.__varios_clase("check_si_color_correcto", kwargs_master = self.master, color_check = self.fg_boton) else "black"



            #se crea el root (toplevel)
            #el toplevel no se muestra aun en pantalla (self.toplevel.withdraw())
            #se muestra una vez configurado sino nada mas crearse aparece durante una fraccion de segundo 
            #el toplevel sin configurar con la pluma tkinter
            self.toplevel = self.objeto_alias_tkinter_import.Toplevel(self.master, bg = bg_messagebox_ajust)
            ...
            self.toplevel.protocol("WM_DELETE_WINDOW", lambda: self.__varios_clase("cerrar_messagebox", boton_valor = None))

            if self.iconbitmap is not None:
                self.toplevel.iconbitmap(self.iconbitmap)


            
            #se crea el frame contenedor
            frame_contenedor = self.objeto_alias_tkinter_import.Frame(self.toplevel, bg = bg_messagebox_ajust, padx = 15, pady = 10)
            frame_contenedor.pack(fill = "both", expand = True)


            #se crea el frame con el mensaje
            frame_msg = self.objeto_alias_tkinter_import.Frame(frame_contenedor, bg = bg_messagebox_ajust)
            frame_msg.pack(fill = "x")


            #se crea un label con imagen personalizada si el kwargs dicc_imagen y el valor de su key png_imagen_messagebox
            #esta bien configurada (es decir que devuelve una ruta correcta de acceso a un fichero png)
            #en caso de que no este bien configurado se asigna como imagen
            #el string del diccionario dicc_config_messagebox (key bitmap) mediante el metodo bitmap
            if self.png_imagen_messagebox is None:

                if bitmap is not None:
                    label_imagen = self.objeto_alias_tkinter_import.Label(frame_msg, bg = bg_messagebox_ajust, bitmap = bitmap)
                    label_imagen.pack(side = "left", padx = (0, 10))

            else:
                try:
                    #se redimensiona la imagen sino el label salen distorsionado en la gui
                    #se usa el kwargs tupla_pixeles_imagen
                    img = Image.open(self.png_imagen_messagebox).resize(self.tupla_pixeles_imagen, Image.LANCZOS)
                    img_tk = ImageTk.PhotoImage(img)

                    label_imagen = self.objeto_alias_tkinter_import.Label(frame_msg, bg = bg_messagebox_ajust)
                    label_imagen.pack(side = "left", padx = (0, 10))
            ...
            #se crea un label con el mensaje
            label_mensaje = self.objeto_alias_tkinter_import.Label(frame_msg, text = self.mensaje, justify = "left", bg = bg_messagebox_ajust, wraplength = 320)
            label_mensaje.pack(side = "left")



            #se crea un frame para encapsular los botones
            frame_botones = self.objeto_alias_tkinter_import.Frame(frame_contenedor, bg = bg_messagebox_ajust, pady = 10)
            frame_botones.pack()

        
            #se configuran los fg y font
            lista_config = [self.toplevel, frame_contenedor, frame_msg, label_imagen, label_mensaje, frame_botones] if bitmap else [self.toplevel, frame_contenedor, frame_msg, label_mensaje, frame_botones]
            for widget in lista_config:

                try:
                    widget.configure(**{"fg": fg_messagebox_ajust})
                    widget.configure(**{"font": self.font_messagebox})

                except:
                    try:
                        widget.configure(**{"font": self.font_por_defecto})
                    except:
                        pass
            ...
            #se crean los botones con el valor asociado al pulsarlo y cerrar el messagebox
            if isinstance(lista_botones, list):

                lista_widget_botones = []
                for boton_texto, boton_valor in lista_botones:

                    widget_boton = (self.objeto_alias_tkinter_import.Button(frame_botones, text = boton_texto, width = 10, bg = bg_boton_ajust, fg = fg_boton_ajust
                                                                    , command = lambda lambda_boton_valor = boton_valor: self.__varios_clase("cerrar_messagebox", boton_valor = lambda_boton_valor)))
                    
                    widget_boton.pack(side = "left", padx = 5)
                    lista_widget_botones.append([boton_texto, widget_boton])

                    try:
                        widget_boton.configure(**{"font": self.font_boton})
                    except:
                        try:
                            widget_boton.configure(**{"font": self.font_por_defecto})
                        except:
                            pass
                ...
                #se marca el foco en el boton segun lo configurado en self.dicc_config_messagebox
                if isinstance(marcar_foco_boton, str):
                    for boton_texto, widget_boton in lista_widget_botones:
                        if marcar_foco_boton == boton_texto:
                            widget_boton.focus_set()
                            break
            ...
            #se centra el messagebox en pantalla
            width = self.toplevel.winfo_width()
            height = self.toplevel.winfo_height()
            coord_x = self.master.winfo_rootx() + (self.master.winfo_width() // 2) - (width // 2)
            coord_y = self.master.winfo_rooty() + (self.master.winfo_height() // 2) - (height // 2)

            self.toplevel.update_idletasks()
            self.toplevel.geometry(f"+{coord_x}+{coord_y}")


            #se genera el toplevel en pantalla tras realizar toda su configuracion
            self.toplevel.update_idletasks()
            self.toplevel.deiconify()
            self.toplevel.focus_force()

            self.master.wait_window(self.toplevel)


        elif opcion == "cerrar_messagebox":
            #cierra el messagebox y asigna el valor del boton pulsado
            #al atributo propio self.valor_boton_pulsado

            self.valor_boton_pulsado = boton_valor
            try:
        ...
        elif opcion == "check_si_color_correcto":
            #permite chequear que el bg / fg (background / foreground) es correcto para poder aplicarlo en la creacion del widget
            #pq cuando se configuran bg / fg se requiere configurar los mismos en el momento de la creacion de los distintos widgets
            #que componen el messagebox (toplevel, frames, labels y botones) en caso contrario
            #en caso contrario el toplevel sale en el gris clasico y el resto de widgets con el bg que se quiere aplicar

            try:
                kwargs_master.winfo_rgb(color_check)
...
class controltiptext:
    ...
    El mensaje aparece en pantalla durante un tiempo determinado (configurable con el atributo autodestroy_tiempo_max),
    esto evita que el mensaje se quede "colgado" a veces en pantalla a pesar de quitar el foco sobre el widget asociado.
    """
    ...
    def __init__(self, widget_objeto, mensaje_controltiptext, delay = 500):
        ...
        self.tipwindow = None
        self.after_id = None
        self.after_id_autodestroy = None
        self.autodestroy_tiempo_max = 3000# 3 segundos
    ...
    def __varios_clase(self, opcion: Literal["objeto_alias_libreria_python_import", "mostrar_mensaje_controltiptext_cuando_focus_widget"], **kwargs):
        ...
        if opcion == "objeto_alias_libreria_python_import":
            ...
            #autodestruccion forzada
            self.after_id_autodestroy = self.tip_ventana.after(self.autodestroy_tiempo_max, self.__destroy_mensaje)
    ...
    def __destroy_mensaje(self):
        """Rutina interna que permite destruir el mensaje pasado un tiempo máximo configurado en el constructor de la clase."""
    ...
    def __varios_clase_event(self, opcion: Literal["tiempo_espera_para_mostrar_mensaje_controltiptext"
        ...
        """
        Rutina interna de evento para usar en los binds de la clase.
        ...
        --> eliminar_mensaje_controltiptext_cuando_focus_out
        \n\nPermite borrar el mensaje cuando el usuario al colocar el cursor del raton sobre el widget lo mueve en otro sitio
        """
        ...
        if opcion == "tiempo_espera_para_mostrar_mensaje_controltiptext":
            ...
            self.after_id = self.widget_objeto.after(self.delay, lambda: self.__varios_clase("mostrar_mensaje_controltiptext_cuando_focus_widget"))
            ...
            try:
                ...
                if self.after_id_autodestroy:
                    self.widget_objeto.after_cancel(self.after_id_autodestroy)
            except:
                pass
